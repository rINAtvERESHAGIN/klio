          ...
          <b-col cols="12" xl="10" offset-xl="1" v-if="loadingStatus === 'success'">
            <b-row v-if="PRODUCT">
              <b-col cols="12" md="5" lg="4" class="mb-5" v-if="PRODUCT.images.length">
                <VueGallery :images="getImages" :index="index" @close="index = null" />
                ...
                <b-row>
                  <b-col cols="12" lg="9">
                    ...
                    <b-badge v-if="PRODUCT.special" class="product-badge badge-special">
                      <b-link :to="{ name: 'special-detail', params: {special_name: PRODUCT.special.slug } }">акция</b-link>
                    </b-badge>
                    <p v-if="PRODUCT.brand" class="product-brand">{{ PRODUCT.brand }}</p>
                    <h1 class="text-upper">{{ PRODUCT.name }}</h1>
                    <div class="product-tag-list">
                      <b-badge v-for="tag in PRODUCT.tags" :key="tag.id" class="tag-badge" :to="`/search?tags=${tag.name}`">{{ tag.name }}</b-badge>
                    </div>
                    ...
                    <b-col cols="12" md="10" lg="8" offset-md="1" offset-lg="4">
                      ...
                      <div v-if="PRODUCT.properties.length" class="product-properties">
                          ...
                          <p v-for="property in PRODUCT.properties" :key="property.id">
                            <span class="product-property-label">{{ property.name }}:</span>{{ property.value }} {{ property.units }}</p>
                      </div>
                    </b-col>
                </b-row>
            </b-col>
    ...
    getBreadcrumbs () {
      if (this.PRODUCT) {
        ...
        if (this.PRODUCT.categories) {
          ...
          pathTo.push({ text: currentCategory.name, to: { name: 'category-products', params: { category_name: currentCategory.slug } } })
...
<style lang="scss">
  ...
  .product-price-row .product-price .product-ammount .form-input {
