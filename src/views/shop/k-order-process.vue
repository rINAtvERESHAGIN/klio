        ...
        <b-col cols="12" lg="8" offset-lg="2">
          <div class="order-status-line">
            <ul>
              ...
              <li><icon class="icon icon-diamond"></icon></li>
            </ul>
          </div>
        </b-col>
      </b-row>
      <b-row>
        <b-col cols="12" md="4" lg="2" order="2" order-lg="1" class="mt-4 mt-lg-0">
          <b-button v-if="ORDER_ACTIVE.step == 1" class="btn-yellow basket-order-btn sticky-top" :to="{ name: 'basket' }">
            <icon class="icon icon-arrow left float-left"></icon>{{ steps.find(step => step.number === ORDER_ACTIVE.step).prevText }}</b-button>
        ...
        <b-col cols="12" md="4" lg="2" offset-md="4" offset-lg="0" order="3" class="mt-4 mt-lg-0">
          <b-button
            ...
            @click="nextStep()"
          >
            {{ steps.find(step => step.number === ORDER_ACTIVE.step).nextText }}<icon class="icon icon-arrow"></icon>
          </b-button>
        </b-col>
      </b-row>
    </b-col>
