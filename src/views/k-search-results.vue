  ...
  <b-row class="content-section">
    <b-col cols="12" md="10" offset-md="1" id="search-content">
      ...
      <p v-if="results">
        ...
        >
          <b-button squared size="lg" class="btn-grey mb-2">{{ results.counts[type.obj] }} {{ type.name }}</b-button>
        </b-link>
      </p>
      <b-row v-if="resultsCurrentType" class="bottom-spaced">
        <b-col cols="8">
          ...
          >
            по названию<icon :class="'icon icon-arrow ' + (direction === 'desc' ? 'down' : 'up')"></icon>
          </b-button>
          <b-button
...
export default {
  ...
  data: function () {
    return {
      ...
      resultsTypes: [
        { obj: '', name: 'Все' },
        { obj: 'categories', name: 'Категории' },
        { obj: 'products', name: 'Товары' },
        { obj: 'articles', name: 'Статьи' },
        { obj: 'news', name: 'Новости' }
      ],
